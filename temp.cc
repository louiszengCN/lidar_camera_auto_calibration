...
using namespace std;
//路径
string image_path = "../data/img_pinhole/2.png";
string pointcloud_path = "../data/pcd_pinhole/2.pcd";
...
// 定义点云类型
typedef pcl::PointXYZ PointT_1;
typedef pcl::PointCloud<PointT_1> PointCloud1;


PointCloud1::Ptr pointCloud(new PointCloud1);  
...
void projectPointCloudToImage(const PointCloud1::Ptr& pointCloud, cv::Mat& image, cv::Mat _rvec, cv::Mat _tvec);
...
void get_ro_tran(double _rx, double _ry, double _rz, double _x, double _y, double _z)
{
    ...
    rvec = rot_vector;
    tvec = tra_vector;
    projectPointCloudToImage(pointCloud, image, rvec, tvec, cameraMatrix, distCoeffs);

}

// 点云投影到图像
// 输入点云 图片
void projectPointCloudToImage(const PointCloud1::Ptr& pointCloud, cv::Mat& image, cv::Mat _rvec, cv::Mat _tvec, cv::Mat cameraMatrix, cv::Mat distCoeffs)
{
...
int manual_adjust(int argc, char *argv[]){
